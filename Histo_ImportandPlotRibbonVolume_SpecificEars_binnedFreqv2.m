...
%% Group the data
% Define the groups
M_long = [108, 109, 110, 114, 124, 125, 119, 120, 122, 123];
%F_long = [];
...
groupNames = {'M_long', 'M_short', 'control', 'specific_ears'};
groups = {M_long, M_short, control, specific_ears}; % Ensure groups are defined
...
for k = 1:length(groupNames)
    ...
    plot(bins, volumes + sds, '--', 'Color', colors(k, :));
    plot(bins, volumes - sds, '--', 'Color', colors(k, :));
end
